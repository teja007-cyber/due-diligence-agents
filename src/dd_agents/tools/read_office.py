...
def read_office(
    ...
    Args:
        ...
        allowed_dir: If set, restrict reads to this directory tree (prevents
            path traversal when called by LLM agents).
    ...
    if allowed_dir:
        try:
            ...
            if not resolved.is_relative_to(allowed_resolved):
                return {"status": "error", "reason": "Path traversal blocked: file is outside the allowed directory"}
